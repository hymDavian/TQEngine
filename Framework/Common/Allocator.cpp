...
void My::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment)
{
	...
	//这种魔术只在对齐为2^n时有效，通常情况下应该是这样
	//因为大多数CPU/GPU也要求对准在2^n
	//但我们仍然用断言来保证
#if defined(_DEBUG)
	//如果 assert 的内容条件返回错误，代码会终止运行，并将代码与行号输出
...
void* My::Allocator::Allocate()
{
	if (!m_pFreeList) {
		//分配一个新页面
		//这里的reinterpret_cast含义为，将()内的指针重新解释为<>内的指针类型,之后实际使用时
		//将旧的指针类型指向的数据，以<>内的新指针类型进行解释。
