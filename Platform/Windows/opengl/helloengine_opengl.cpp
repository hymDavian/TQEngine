...
//DEFINES
//一些需要预定义的宏，由OpenGL规范定义，对具体业务无实际意义，但要使用opengl就必须写
...
//类型别名
//WINAPI本质是保留关键字 "__stdcall" 的别名
//被 __stdcall 修饰的函数，是一种函数调用约定
//参数从右向左压入堆栈
//函数被调用者修改堆栈
//函数名自动添加前导下划线，后面紧跟一个@符号
//typedef retType(*name)(...paramstype)  类似于 ts的  type name = (...paramstype)=>retType,规定一种函数对象的参数结构

//以下这些函数别名跟上面的宏同理，只是规定需要
...
//使用上面的这些函数指针类型 定义我们具体需要用的指针，用以存储OpenGLAPI的调用地址
...
//vsshader和psshader的程序文件名，以便之后运行时读取这俩个文件的内容，并热编译成需要的shader对象
//但是，这样做有一定安全隐患性，且PS平台上不允许这么做
...
//MVP矩阵，实现3D空间到2D空间的投影，也就可以实现一些3D动画效果。
//例如一个方块，我们可以在一个平面上看出这是一个方块
//就是因为这个图案画面各个像素颜色呈现了一个方块的透视图
//而这个图的形成就是基于此MVP矩阵
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	//DrawContext(DC)
	//由于OpenGL的api调用，必须是基于已创建的一个DrawContext的环境下
	//因此必须先创建一个临时窗口用以创建一个这样的DC环境，将OpenGLAPI的入口地址全部加载到定义好的函数指针中

	//用所需信息填充结构
	wc.cbSize = sizeof(WNDCLASSEX);
	//CS_OWNDC指示window为该窗口类的每个窗口实例单独创建DC,
	//避免多个窗口共用资源导致的一些奇怪BUG，所里这里指示使用单独的环境，将各个窗口区分开来
	...
	/*
	由于在调用wgl系的函数之前，要先确定frame buffer的pixel format，
	也就是frame buffer的格式，
	InitializeExtensions里面，指定了缺省的pixel format。
	*/
	InitializeExtensions(hWnd);

	//这里注意，相同的 window class(也即是本代码文件) 的窗口是公用一个消息处理函数 (windowproc)
	//如果我们在创建两次窗口的时候，第二次窗口紧跟着第一个窗口销毁之后，第二个窗口很可能会在收到第一个窗口被销毁的消息
	//会导致第二个窗口立即被关闭
	DestroyWindow(hWnd);
	hWnd = NULL;
	...
	/*
	读入shader程序，并对其进行编译和绑定
	指设定GPU相关的寄存器，使其指向我们的程序在内存当中的地址，
	以及绑定Shader程序（在这个例子里是VS Shader）输入参数的格式。
	在这个例子里 参数的格式 就是我们VertexType的两个属性: position, color
	*/
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
		/*
		好了，接下来就是计算绑定MVP了。MVP的特点是每帧更新一次（当然也可以不变），
		在单帧的绘制过程当中，它是一个常数。
		所以，MVP在GPU当中存放的地方，也叫Constant Buffer。
		这个Constant，就是指在一帧当中，它们是常数（同样的还有光照数据等）
		*/
		Draw();
		return 0;
	}
